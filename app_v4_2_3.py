...
# ---------- UI: Tabs ----------
tab_build, tab_transform, tab_expr = st.tabs([
    "1) Build distributions (with results)",
    "2) Transform (Single | All)",
    "3) Custom function (combine)"
])

# ===== Build Tab =====
with tab_build:
    st.header("Build Distributions (A–D)")
    st.caption("Choose type and parameters per A–D. Optional truncation via rejection sampling. After generating, see the summary table and plots here.")
...
with tab_transform:
    st.header("Transform (Single | All) — v4.2.3")
    ...
    if "data_dict" not in st.session_state:
        st.info("Please generate A–D in the first tab.")
        st.stop()
...
with tab_expr:
    st.header("Custom function (combine) — v4.2.3")
    st.caption("Build a new series Y from A–D using a safe expression. Operators: + - * / ** ( ) and functions: abs, exp, log, log10, sqrt, power, clip, minimum, maximum, sin, cos, tan, where. Constants: pi, e, and 'eps'. '^' is accepted as power.")
    ...
    with col1:
        expr_text = st.text_input("Expression (e.g., Y = A*B / C**4,  (A+B)/2,  clip(A,-1,1),  power(A, 0.25))", value="Y = A*B / (C**2 + eps)")
    with col2:
